...
function sendMailIndex($hoten,$tieude,$body,$emailAddress=null,$emailCC=null,$emailBCC=null){
	global $config_ip,$config_email,$config_pass,$row_setting,$lang;
	...
	if(!empty($emailAddress)){
		foreach ($emailAddress as $k => $v) {
			$mail->AddAddress($v,$row_setting['ten_'.$lang]);
		}
	}
	if(!empty($emailCC)){
		foreach ($emailCC as $k => $v) {
			$mail->AddCC($v,$row_setting['ten_'.$lang]);
		}
	}
	if(!empty($emailBCC)){
		foreach ($emailBCC as $k => $v) {
			$mail->AddBCC($v,$row_setting['ten_'.$lang]);
		}
	}

	$mail->Subject    = $tieude." - ".$hoten;
	$mail->IsHTML(true);
	$mail->CharSet = "utf-8";
	$mail->Body = $body;
	if($mail->Send()){
...
function getLink($str){
	return '<a href="'.changeTitle($str).'.html" title="'.$str.'">'.$str.'</a>';
}
...
function get_thuonghieu($pid){
	global $d, $row,$lang;
	$sql = "select ten_$lang from #_product_list where id='".$pid."'";
	$d->query($sql);
	$row = $d->fetch_array();
...
function checkPermission(){
	global $d;
	$sql = "select id,secretkey from #_user where id='".$_SESSION['login']['id']."'";
	$d->query($sql);
	$row = $d->fetch_array();
...
function phanquyen_tv($com,$quyen,$act,$type){
	...
	if($act == 'capnhat' || $act == 'man' || $act == 'man_list' || $act == 'man_cat' || $act == 'man_item'){
		if(in_array($com_manager['id'].'|1',$xem_s)){
			return 1;
		} else {
			return 0;
		}
	} elseif($act == 'add' || $act == 'add_cat' || $act == 'add_list' || $act == 'add_item') {
		if(in_array($com_manager['id'].'|1',$them_s)){
			return 1;
		} else {
			return 0;
		}
	} elseif($act == 'delete' || $act == 'delete_list' || $act == 'delete_cat' || $act == 'delete_item'){
		if(in_array($com_manager['id'].'|1',$xoa_s)){
			return 1;
		} else {
			return 0;
		}
	} elseif($act == 'edit' || $act == 'edit_list' || $act == 'edit_cat' || $act == 'edit_item'){
...
function giamgia($giagoc,$giagiam)
{	
	$ketqua = 100 - ($giagiam*100)/($giagoc);
	$phantram = round($ketqua).'%';
	return '-'.$phantram;	
}
...
function pagination($query,$per_page=10,$page=1,$url='?'){   
    ...
    if($lastpage > 1){   
        $pagination .= "";
        $pagination .= "<a class='page gradient'>Page {$page} of {$lastpage}</a>";
              
            if ($page > 1) $pagination.= "<a class='page gradient' href='{$url}&page={$prev}'>{$prevlabel}</a>";
              
        if ($lastpage < 7 + ($adjacents * 2)){   
            for ($counter = 1; $counter <= $lastpage; $counter++){
                if ($counter == $page)
                    $pagination.= "<a class='page active'>{$counter}</a>";
                else
                    $pagination.= "<a class='page gradient' href='{$url}&page={$counter}'>{$counter}</a>";                    
            }
            ...
            if($page < 1 + ($adjacents * 2)) {
                ...
                $pagination.= "<a class='page gradient' href='{$url}&page=1'>...</a>";
                $pagination.= "<a class='page gradient' href='{$url}&page={$lpm1}'>{$lpm1}</a>";
                $pagination.= "<a class='page gradient' href='{$url}&page={$lastpage}'>{$lastpage}</a>";  
                      
            } elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2)) {
                  
                $pagination.= "<a class='page gradient' href='{$url}&page=1'>1</a>";
                $pagination.= "<a class='page gradient' href='{$url}&page=2'>2</a>";
                $pagination.= "<a class='page gradient' href='{$url}&page=1'>...</a>";
            ...
            if ($page < $counter - 1) {
                $pagination.= "<a class='page gradient' href='{$url}&page={$next}'>{$nextlabel}</a>";
                $pagination.= "<a class='page gradient' href='{$url}&page=$lastpage'>{$lastlabel}</a>";
            }
