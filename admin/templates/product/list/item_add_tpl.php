...
<div class="control_frm">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=product&act=add_list<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span><?=$config_cap1['title']?></span></a></li>
...
<form name="supplier" id="validate" class="form" action="index.php?com=product&act=save_list<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
		...
		<div class="<?=($config_cap1['full']==true) ? 'oneOne':'colLeft' ?>">
			<div class="widget mtop0">
				...
				<div class="formRow lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?> none">
					<label>Tiêu đề 2 [<?=$v?>]</label>
					<div class="formRight">
		                <input type="text" name="name_<?=$k?>" title="Nhập Tiêu đề 2" id="name_<?=$k?>" class="tipS validate[required]" value="<?=@$item['name_'.$k]?>" />
					</div>
					<div class="clear"></div>
				</div>
				...
				<div class="formRow lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
					<label>Mô tả [<?=$v?>]</label>
					<div class="ck_editor">
		                <textarea title="Nhập mô tả . " id="mota_<?=$k?>" <?=($config_cap1['mota-ckeditor']==true) ? 'class="ck_editors"':'rows="8"' ?> name="mota_<?=$k?>"><?=@$item['mota_'.$k]?></textarea>
					</div>
					<div class="clear"></div>
				</div>
				<?php } ?>
		...
		<div class="<?=($config_cap1['full']==true) ? 'oneOne':'colRight' ?>">
			<div class="widget mtop0">
				...
				<div class="formRow">
					...
					<div class="mt10"><img src="<?=_upload_product.$item['thumb']?>"  alt="NO PHOTO" width="40" /></div>
					</div>
					<div class="clear"></div>
				</div>
				<?php } ?>
				...
				<?php if($config_cap1['color']==true){ ?>
				<div class="formRow">
					<label>Màu</label>
					<div class="formRight">
		                <input type="color" name="mau" title="Nhập màu" id="mau" class="tipS" value="<?=@$item['mau']?>" />
					</div>
					<div class="clear"></div>
				</div>
				<?php } ?>
				...
				<div class="formRow">
					...
					<div class="mt10"><img src="<?=_upload_product.$item['quangcao']?>"  alt="NO PHOTO" width="316" /></div>
					</div>
					<div class="clear"></div>
				</div>
				<?php } ?>
				...
				<div class="formRow none">
					<label>Links</label>
					<div class="formRight">
		                <input type="text" name="links" title="Nhập links" id="links" class="tipS" value="<?=@$item['links']?>" />
					</div>
					<div class="clear"></div>
				</div>
				<?php } ?>
	...
	<div class="formRow fixedBottom">
		<div class="formRight">
        	...
        	<a href="index.php?com=product&act=man_list<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" onClick="if(!confirm('Bạn có muốn thoát không ? ')) return false;" title="" class="button tipS redB" original-title="Thoát">Thoát</a>
		</div>
	</div>
</form>
</div>
